...
class ResultBuffer:
    ...
    async def _flush(self) -> None:
        ...
        await self._generate_findings(snapshot)

    async def _generate_findings(self, snapshot: dict[str, dict]) -> None:
        """Run finding engine on completed tasks after flush."""
        completed_ids = [
            tid for tid, p in snapshot.items()
            if p.get("status") in ("completed", "ok")
        ]
        ...
        try:
            ...
            total = await asyncio.to_thread(_analyze)
            if total > 0:
                logger.info("Auto-generated %d findings from %d completed tasks.", total, len(completed_ids))
        except Exception:
            logger.exception("Auto-finding generation failed")
