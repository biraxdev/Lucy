...
async def _auth_agent(agent_id: str) -> dict | None:
    """Authenticate an agent by its registered ID in the database."""
    if not agent_id:
        return None
    try:
        ...
        if agent:
            return {"id": agent_id, "role": "agent", "hostname": agent.hostname}
    except Exception:
        pass
    return None
...
async def websocket_agent(
    ...
    Auth: ?token=<jwt> OR ?api_key=<hex> OR agent_id match in DB
    Channel: agent:{agent_id}
    """
    user = await _auth_jwt(token) or await _auth_api_key(api_key) or await _auth_agent(agent_id)
    if user is None:
        await websocket.close(code=4001)
        return
...
async def _on_screen_frame(agent_id: str, msg: dict) -> None:
    ...
    if isinstance(payload, dict):
        frame = payload.get("data") or payload.get("frame")
    else:
        frame = msg.get("data") or payload
    await manager.broadcast_to_frontends(
        build_message(
            "screen_frame",
            {"frame": frame, "data": frame, "frame_number": (payload.get("frame") if isinstance(payload, dict) else 0)},
            agent_id=agent_id,
        ),
...
manager.register_handler("screenshot_frame", _on_screen_frame)
