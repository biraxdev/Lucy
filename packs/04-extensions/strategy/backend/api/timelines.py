...
async def list_templates(current_user: CurrentUser) -> list[dict]:
    """Return all built-in attack plan templates."""
...
async def import_template(template_id: str, current_user: OperatorUser) -> dict:
    """Create a new Timeline from a built-in template."""
    tpl = next((t for t in ATTACK_TEMPLATES if t["id"] == template_id), None)
    if not tpl:
    ...
    with database:
        tl = Timeline.create(
            ...
            agent_group=json.dumps(tpl["agent_group"]),
