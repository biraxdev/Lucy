...
export default function Dashboard() {
  ...
  const { data: libraryStats } = useQuery({
    queryKey: ['dashboard-library-stats'],
    queryFn: () => api.get<{ total_resources: number }>('/library/stats').then(r => r.data).catch(() => ({ total_resources: 0 })),
  ...
  const connLine = useMemo(() => {
    ...
    // No fake data — return empty array so the chart shows "no data" state
    return []
  }, [connHistory])
      ...
      <div className="page-container space-y-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="space-y-2">
            ...
            <ViewLabel type="macro" label="Operations Center" />
          </div>
        ...
        <section className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <Card className="p-5" hover onClick={() => navigate('/library')}>
              <CardMetric value={libraryStats?.total_resources ?? 0} label="Library" trend="resources" color="text-info" />
            </Card>
          </div>
        </section>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold flex items-center gap-2"><Radio size={18} className="text-success animate-pulse" /> Endpoint Status</h2>
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <QuickAction icon={Zap} label="Run Recon" description="Execute default reconnaissance" color="success" onClick={handleQuickRecon} />
            <QuickAction icon={Terminal} label="Dispatch Shell" description="Send shell command to selected agents" color="info" onClick={() => navigate('/terminal')} />
            <QuickAction icon={Bell} label="Review Alerts" description="Investigate unread notifications" color="warning" onClick={() => navigate('/alerts')} />
            <QuickAction icon={FileText} label="Generate Report" description="Export current operations summary" color="ghost" onClick={() => navigate('/reports')} />
          </div>
        </section>
      </div>
    </PageTransition>
