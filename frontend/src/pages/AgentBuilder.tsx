...
import { getBuildPacks } from '../api/build'
...
function BuildPackSelector({ onApply }: { onApply: (pack: BuildPack) => void }) {
  const { data: packs = [], isLoading } = useQuery({ queryKey: ['build-packs'], queryFn: getBuildPacks })
  return (
    <Card hover={false} className="p-4 space-y-3">
      <h3 className="font-semibold text-sm flex items-center gap-2"><Package size={16} className="text-success" /> Build Packs</h3>
      <p className="text-xs text-base-content/50">Apply a preset to quickly select modules and build options.</p>
      {isLoading ? (
        <span className="loading loading-spinner loading-sm text-success" />
      ) : (
        <div className="flex flex-wrap gap-2">
          {packs.map((pack: BuildPack) => (
            <button
              key={pack.bpid}
          ...
          {packs.length === 0 && <p className="text-xs text-base-content/40">No build packs available.</p>}
        </div>
      )}
    </Card>
  )
...
export default function AgentBuilder() {
  ...
  const [buildMode, setBuildMode] = useState<'standard' | 'quick'>('standard')
  ...
  const applyBuildPack = (pack: BuildPack) => {
    const packModules = pack.modules.filter((m) => ALL_MODULES.some((am) => am.id === m))
    setSelectedModules(packModules.length > 0 ? packModules : ALL_MODULES.map((m) => m.id))
    const overrides = pack.build_options || {}
    setOpts((prev) => ({ ...prev, ...overrides }))
  }
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url
      // Use the artifact_type from the backend response, not the frontend mode
      const ct = String(res.headers['content-type'] || '')
      const isZip = artifactType === 'zip' || ct.includes('zip')
      const ext = isZip ? '.zip' : (os === 'windows' ? '.exe' : '')
      a.download = `${opts.custom_name}${ext}`
      a.click()
  ...
  return (
    <PageTransition>
      <div className="page-container max-w-5xl space-y-6">
        ...
        {/* === Build Packs === */}
        <BuildPackSelector onApply={applyBuildPack} />
