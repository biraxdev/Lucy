        ...
        {(task.status === 'completed' || task.status === 'failed') && !compact && (
          <div className={clsx(
            ...
            {task.status === 'completed'
              ? previewResult(task.result, task.module)
              : task.error || 'Task failed'}
          </div>
        )}

        {/* Thumbnail for screenshot/webcam results */}
        {task.status === 'completed' && !compact && hasImageResult(task.result, task.module) && (
          <div className="mt-2 rounded-lg overflow-hidden border border-base-content/10">
            <img
              src={`data:image/jpeg;base64,${extractImageB64(task.result)}`}
              alt={`${task.module} thumbnail`}
...
function previewResult(result: any, module?: string): string {
  if (!result) return 'No output'
  // For screenshot/webcam modules, show a friendly label instead of base64
  if (module && ['screenshot', 'webcam', 'screen_stream'].includes(module)) {
    if (hasImageResult(result, module)) return `📸 ${module} captured (click to view)`
  }
  try {
...
/** Check if a task result contains a base64 image */
function hasImageResult(result: any, module?: string): boolean {
  if (!result || !module) return false
  if (!['screenshot', 'webcam', 'screen_stream'].includes(module)) return false
  return !!extractImageB64(result)
}

/** Extract base64 image data from a task result */
function extractImageB64(result: any): string | null {
