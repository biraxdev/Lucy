...
export default function OnboardingWizard({ onClose }: Props) {
  ...
  const agentCmd = `# Option 1 — Python direct (capteur défensif)
python agent.py --mode defensive
      ...
      <div className="relative w-full max-w-2xl bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          <div className="flex items-center gap-3">
            <span className="text-green-400 font-bold text-lg">Lucy Defense Platform</span>
        ...
        <div className="px-6 py-6 min-h-[300px]">
          ...
          {step === 1 && (
            <div className="space-y-5">
              <div>
                <h2 className="text-2xl font-bold text-white mb-1">Bienvenue sur Lucy Defense Platform</h2>
                <p className="text-gray-400">Plateforme de surveillance défensive et de détection pour environnements autorisés.</p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { icon: '�️', title: 'Defense Center', desc: 'Ingestion SIEM, alertes et détection MITRE ATT&CK.' },
                  { icon: '�', title: 'Scanners autorisés', desc: 'Découverte réseau, intégrité fichiers, anomalies processus.' },
                  { icon: '⚡', title: 'Surveillance temps réel', desc: 'Agents capteurs, tâches et logs centralisés.' },
          ...
          {step === 3 && (
            <div className="space-y-5">
              <div>
                <h2 className="text-xl font-bold text-white mb-1">Déployer un capteur</h2>
                <p className="text-gray-400 text-sm">Installe le capteur autorisé sur un hôte géré pour envoyer des événements au Defense Center.</p>
              </div>
              ...
              <div className="bg-gray-800 rounded-xl border border-gray-700 p-4 space-y-2">
                <div className="text-gray-300 text-sm font-semibold mb-2">URL backend détectée</div>
                <div className="font-mono text-green-400 text-sm bg-gray-900 rounded px-3 py-2">{c2Url}</div>
                <p className="text-gray-500 text-xs">Le capteur doit pouvoir joindre cette adresse depuis l'hôte surveillé.</p>
              </div>
