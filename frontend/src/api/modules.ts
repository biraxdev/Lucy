...
import type { Module, ModuleDescriptor } from '../types/module'
...
export const getModuleDescriptor = (id: string) =>
  api.get<ModuleDescriptor>(`/modules/${id}/descriptor`).then(r => r.data)
...
export const uploadModuleManifest = (file: File) => {
  const formData = new FormData()
  formData.append('file', file)
  return api.post<Module>('/modules/upload-manifest', formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  }).then(r => r.data)
}
